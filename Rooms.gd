...
const SPAWN_ROOMS: Array =[
	preload("res://rooms/SpawnRoom.tscn")
]
const INTERMEDIATE_ROOMS:Array = [
	preload("res://rooms/Room0.tscn")
]
const END_ROOMS:Array  =[
	preload("res://rooms/test/EndRoom1.tscn")
]

const TILE_SIZE:int = 16
const FLOOR_TILE_INDEX:int = 11
const RIGHT_WALL_TILE_INDEX = 37
const LEFT_WALL_TILE_INDEX = 36
...
func _spawn_rooms()->void:
	...
	for i in num_levels:
		...
		if i == 0:
			...
			if i == num_levels - 1:
				room = END_ROOMS[randi()% SPAWN_ROOMS.size()].instance()
			else:
			...
			room.position = previous_room_door.global_position + Vector2.UP * room_tilemap.get_used_rect().size.y * TILE_SIZE + Vector2.UP * (1 + corridor_height) * TILE_SIZE + Vector2.LEFT * room_tilemap.world_to_map(room.get_node("Enterance/Position2D2").position).x * TILE_SIZE
