...
func _ready()->void:
	if not on_floor:
		player_detect.set_collision_mask_bit(0,false)
		player_detect.set_collision_mask_bit(1,false)
...
func _on_PlayerDetect_body_entered(body:KinematicBody2D)->void:
	if body != null:
		player_detect.set_collision_mask_bit(0,false)
		player_detect.set_collision_mask_bit(1,false)
		...
		assert(__)
		player_detect.set_collision_mask_bit(1,true)

func interpolate_pos(init_pos:Vector2,final_pos:Vector2)->void:
	var __ = tween.interpolate_property(self,"position",init_pos,final_pos,0.8,Tween.TRANS_QUART,Tween.EASE_OUT)
	assert(__)
	__ = tween.start()
	assert(__)
	player_detect.set_collision_mask_bit(0,true)
